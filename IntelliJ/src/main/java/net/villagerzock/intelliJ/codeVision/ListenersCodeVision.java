...
public class ListenersCodeVision extends CodeVisionProviderBase {
    @Override
    ...
    public @Nls @Nullable String getHint(@NotNull PsiElement psiElement, @NotNull PsiFile psiFile) {
        ...
        if (psiElement instanceof PsiMethod method){
            List<PsiElement> listeners = findAllListenersFor(method,"addListener");
            amountOfListeners = listeners.size();
        }
        return amountOfListeners == 0 ? "no Listeners" : amountOfListeners == 1 ? "1 Listener" : amountOfListeners + " Listeners";
    }
    ...
    public static @NotNull List<PsiElement> findAllListenersFor(@NotNull PsiMethod method, String methodName) {
        ...
        for (PsiReference ref : ReferencesSearch.search(field, scope).findAll()) {
            ...
            // ==> hier: Listener-Argument statt Call zurückgeben
            PsiExpression[] args = call.getArgumentList().getExpressions();
            if (args.length == 0) continue;

            PsiElement listenerArg = args[0];

            if (listenerArg instanceof PsiMethodReferenceExpression mr){
                PsiElement res = mr.resolve();
                if (res != null){
                    listenerArg = res;
                }
            }
            ...
            // dedupe
            PsiFile file = listenerArg.getContainingFile();
            if (file == null) continue;

            String key = file.getVirtualFile() + ":" + listenerArg.getTextRange();
            if (seen.add(key)) out.add(listenerArg);
        }
    ...
    public void handleClick(@NotNull Editor editor, @NotNull PsiElement psiElement, @Nullable MouseEvent mouseEvent) {
        if (psiElement instanceof PsiMethod method && mouseEvent != null){
            List<PsiElement> listeners = findAllListenersFor(method,"addListener");
            navigateToTargets(RelativePoint.fromScreen(mouseEvent.getPoint()),listeners,"Listener");
        }
    }

    public static void navigateToTargets(@NotNull RelativePoint point,
                                    @NotNull Collection<? extends PsiElement> targets,
                                    @NotNull String popupTitle) {
        if (targets.isEmpty()) return;

        if (targets.size() == 1) {
            PsiElement only = targets.iterator().next();
            if (only instanceof Navigatable nav && nav.canNavigate()) {
                nav.navigate(true);
            }
            return;
        }
    ...
    public @Nls @NotNull String getName() {
        return "Listeners of this Event";
    }
    ...
    public @NotNull String getId() {
        return "net.villagerzock.listeners";
    }
}
